...
router.delete("/experience/:exp_id", auth, async (req, res) => {
  try {
    ...
    //get index.
    const indextToRemove = profile.experience
      .map((item) => {
        item.id;
      })
...
router.delete("/education/:edu_id", auth, async (req, res) => {
  try {
    ...
    //get index.
    const indextToRemove = profile.education
      .map((item) => {
        item.id;
      })
      .indexOf(req.params.id);
  ...
  try {
    ...
    const options = {
      uri: `https://api.github.com/users/${
        req.params.username
      }/repos?per_page=8&sort=:asc&client_id=${config.get("GITHUB_CLIENT_ID")}
